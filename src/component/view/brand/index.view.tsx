...
export default function ViewBrandIndex() {
  ...
  const {
    ...
    queryKey: ['brand-list', $page, $search_query],
    queryFn: () =>
      api_brand_find({
        page: $page,
        take: 3,
        brand_name: $search_query ? [$search_query] : undefined,
        order_by: ['-brand_id'],
        columns: ['brand_id', 'brand_name', 'brand_is_boycotted', 'created_at'],
      }),
    retry: false,
  })
  ...
  useEffect(() => {
    if ($brands_data?.data) {
      const new_items = $brands_data.data
      $_pages_data((prev) => ({
        ...prev,
        [$page]: new_items,
      }))
  ...
  const brand_items = Object.keys($pages_data)
    .map(Number)
    .sort((a, b) => a - b)
    .flatMap((pageNum) => $pages_data[pageNum] || [])
    .filter((item) => {
      if (seen_ids.has(item.brand_id)) {
        return false
      }
      seen_ids.add(item.brand_id)
      return true
    })
      ...
      <Stack gap="xl" className="max-w-[28rem] w-full">
        <Stack gap="xs" className="text-center">
          ...
          <Text size="sm" c="dimmed" className="font-medium">
            Browse and search our catalog of brands.
          </Text>
        </Stack>
          ...
          {$show_initial_loader ? (
            ...
            <>
              {brand_items.map((item) => (
                <div
                  key={item.brand_id}
                  onClick={() => navigate({ to: '/brand/$brand_id', params: { brand_id: String(item.brand_id) } })}
                  className="cursor-pointer"
                >
                  <CardBrand brand_name={item.brand_name || ''} brand_is_boycotted={!!item.brand_is_boycotted} />
                </div>
              ))}
